<template v-on:nav-event="getPos" v-model="oblastModel">
  <div>
    ...
    >
      ...
      <p
        class="text"
      >Якщо ви досі ще не обрали лікаря для себе та вашої родини. Ви можете зробити це за допомогою нашого інструменту. Ви можете знайти лікарів, що працюють у вашому місті в таблиці нижче. Та підібрати собі того хто ще не підписав максимум декларацій з пацієнтами. Для для педіатра максимальна кількість пацієнтів — 900, для сімейного лікаря — 1800 та терапевта — 2000.</p>
    </div>
      ...
      <div class="background">
        ...
        >
          <template slot="top-row">
            <td
              role="cell"
              data-label="Ім'я лікаря"
              aria-colindex="1"
              :style="{'border-radius': 0}"
            >
              <div class="inputColumnName">
                <input
                  v-model="filters['doctor_full_name']"
                  placeholder="шукати"
          ...
          <template slot="top-row">
            <td role="cell" data-label="Адреса" aria-colindex="1" :style="{'border-radius': 0}">
              <div class="inputColumnName">
                <input
                  v-model="filters['division_address']"
                  placeholder="шукати"
                  :style="{'border': `1px solid #b2b2b2`}"
                />
              </div>
    ...
    combinedData() {
      ...
      this.rows.forEach(d => {
        try {
          ...
          t["full_name"] =
            that.get_hospital_name.get(t.le_transfer).le_name +
            ", " +
            t.le_transfer;
          m.push(t);
        } catch {}
